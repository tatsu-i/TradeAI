...
extern "C" int __declspec(dllexport) __stdcall publish_message(
	...
	return _publish_message(
		host, port, user,  password,symbolname, timestamp,
		currentbar, open, close, high, low, label,
...
extern "C" int __declspec(dllexport) __stdcall backtest_message(
	...
	int action
		...
		return _backtest_message(
			host, port, user, password, 
			symbolname, timestamp, expname,
			netprofit, marketposition, 
			OpenPositionProfit, BarsSinceEntry,
			action
		);
};
